...
/*   Updated: 2018/08/17 15:33:50 by rpinoit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	normalize_coord(t_list *room)
{
	...
	while (room != NULL)
	{
		pos = &((t_room *)room->content)->pos;
		pos->x = WIN_W / 10
			+ ((double)(pos->x - ext.min.x)
			/ (double)(ext.max.x - ext.min.x))
			* (double)(WIN_W - (WIN_W / 10 * 2));
		pos->y = WIN_H / 10
			+ ((double)(pos->y - ext.min.y)
			/ (double)(ext.max.y - ext.min.y))
			* (double)(WIN_H - (WIN_H / 10 * 2));
		room = room->next;
	}
}
